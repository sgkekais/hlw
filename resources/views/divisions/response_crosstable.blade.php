                        ...
                        <td class="align-middle text-center">
                            ...
                            @if($fixture)
                                @if($fixture->isPlayed() && !$fixture->isRated())
                                    <span class="badge badge-pill {{ $club_a->hasWon($fixture) ? "badge-success" : null  }} {{ $club_a->hasDrawn($fixture) ? "badge-secondary" : null }} {{ $club_a->hasLost($fixture) ? "badge-danger" : null }}" style="font-size: 100%">{{ $fixture->goals_home }}:{{ $fixture->goals_away }}</span>
                                @elseif($fixture->isRated())
                                    <span class="badge badge-pill badge-warning" style="font-size: 100%">{{ $fixture->goals_home_rated }}:{{ $fixture->goals_away_rated }}</span>
                                @else
                                    <span class="badge badge-pill" style="font-size: 100%">-:-</span>
                                @endif
                            @else
                                <span class="badge badge-pill" style="font-size: 100%">-:-</span>
                            @endif
